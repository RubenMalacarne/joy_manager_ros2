...
public:
    RadiomasterNode() : Node("radiomaster_joystick_controller")
    {
        ...
        switch_SC_labels_[2] = this->get_parameter("switch_SC_low_label").as_string();    // LOW
        switch_SC_labels_[1] = this->get_parameter("switch_SC_center_label").as_string(); // CENTER
        switch_SC_labels_[0] = this->get_parameter("switch_SC_high_label").as_string();   // HIGH

        // Get switch SB position labels
        switch_SB_labels_[2] = this->get_parameter("switch_SB_low_label").as_string();    // LOW
        switch_SB_labels_[1] = this->get_parameter("switch_SB_center_label").as_string(); // CENTER
        switch_SB_labels_[0] = this->get_parameter("switch_SB_high_label").as_string();   // HIGH
